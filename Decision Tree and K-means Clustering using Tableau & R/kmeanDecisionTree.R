...
View(titanic)
summary(titanic)
write.csv (titanic, file = "C:/RStudio/titanicNew.csv")
decision_tree<-titanic
SibSpCat=ifelse(decision_tree$SipSp >=3 , ">=3", "<3")
...
titanicNew<-read.csv("C:/RStudio/titanicNew.csv")
...
write.csv(titanicUpdated,file = "C:/RStudio/titanicUpdated.csv")
...
for(i in 2 : 15)
{
  ...
  btwss[i]<-temp$between
}
plot(totwss, xlab="Number of Cluster", type = "b", ylab = "Total within Sum of Square")
plot(btwss, xlab="Number of Cluster", type = "b", ylab = "Total Between within Sum of Square")
